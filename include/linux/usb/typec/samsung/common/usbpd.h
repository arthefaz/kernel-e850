...
struct usbpd_data {
	...
	int							pd_support;
	int							dp_enabled;
	int							role_reverse_check;
	/* struct wakeup_source wakeup_src; */
	struct wakeup_source		*usbpd_ws;
...
extern void	usbpd_wake_lock(struct wakeup_source *ws);
extern void	usbpd_wake_unlock(struct wakeup_source *ws);
extern int	usbpd_set_wake_lock(struct usbpd_data *pd_data);

extern void	usbpd_manager_vbus_turn_on_ctrl(struct usbpd_data *pd_data, bool enable);
extern int	usbpd_manager_psy_init(struct usbpd_data *pd_data, struct device *parent);
