...
{
	struct s2mu106_dev *s2mu106 = irq_get_chip_data(data->irq);
	const struct s2mu106_irq_data *irq_data = irq_to_s2mu106_irq(s2mu106,
								     data->irq);
...
static irqreturn_t s2mu106_irq_thread(int irq, void *data)
{
	...
	if (irq_src & S2MU106_IRQSRC_AFC) {
		s2mu106_read_reg(s2mu106->muic,
				 S2MU106_REG_AFC_INT, &irq_reg[AFC_INT]);
	...
	if (irq_src & S2MU106_IRQSRC_MUIC) {
		s2mu106_read_reg(s2mu106->muic,
				 S2MU106_REG_MUIC_INT1, &irq_reg[MUIC_INT1]);
		s2mu106_read_reg(s2mu106->muic,
				 S2MU106_REG_MUIC_INT2, &irq_reg[MUIC_INT2]);
	...
	if (irq_src & S2MU106_IRQSRC_PM) {
		s2mu106_read_reg(s2mu106->muic,
				 S2MU106_PM_VALUP1, &irq_reg[PM_VALUP1]);
		s2mu106_read_reg(s2mu106->muic,
				 S2MU106_PM_VALUP2, &irq_reg[PM_VALUP2]);
		s2mu106_read_reg(s2mu106->muic,
				 S2MU106_PM_INT1, &irq_reg[PM_INT1]);
		s2mu106_read_reg(s2mu106->muic,
				 S2MU106_PM_INT2, &irq_reg[PM_INT2]);

		pr_info("%s: powermeter interrupt"
