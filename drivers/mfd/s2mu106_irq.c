...
static struct i2c_client *get_i2c(struct s2mu106_dev *s2mu106, enum s2mu106_irq_source src)
{
	...
	case MST_INT:
		return s2mu106->muic;
#endif
#if IS_ENABLED(CONFIG_MOTOR_S2MU106)
	case HAPTIC_INT:
...
static void s2mu106_irq_sync_unlock(struct irq_data *data)
{
	...
	for (i = 0; i < S2MU106_IRQ_GROUP_NR; i++) {
		...
		if (i >= 0 && i < ARRAY_SIZE(s2mu106_mask_reg))
		...
		s2mu106_write_reg(i2c, s2mu106_mask_reg[i], s2mu106->irq_masks_cur[i]);
	}

	mutex_unlock(&s2mu106->irqlock);
}

static const inline struct s2mu106_irq_data *irq_to_s2mu106_irq(struct s2mu106_dev *s2mu106, int irq)
{
...
{
	struct s2mu106_dev *s2mu106 = irq_get_chip_data(data->irq);
	const struct s2mu106_irq_data *irq_data = irq_to_s2mu106_irq(s2mu106, data->irq);
...
static irqreturn_t s2mu106_irq_thread(int irq, void *data)
{
	...
	if (irq_src & S2MU106_IRQSRC_PM) {
		...
		pr_info("%s: powermeter interrupt(0x%02x, 0x%02x, 0x%02x, 0x%02x)\n",
			__func__, irq_reg[PM_VALUP1], irq_reg[PM_VALUP2], irq_reg[PM_INT1], irq_reg[PM_INT2]);
	}
#endif
#if IS_ENABLED(CONFIG_MST_S2MU106)
	if (irq_src & S2MU106_IRQSRC_MST)
		pr_info("%s: S2MU106_IRQSRC_MST\n", __func__);
#endif
#if IS_ENABLED(CONFIG_MOTOR_S2MU106)
	if (irq_src & S2MU106_IRQSRC_HAPTIC)
		pr_info("%s: S2MU106_IRQSRC_HAPTIC\n", __func__);
#endif
#if IS_ENABLED(CONFIG_REGULATOR_S2MU106)
...
int s2mu106_irq_init(struct s2mu106_dev *s2mu106)
{
	...
	pr_err("%s:%s irq=%d, irq->gpio=%d\n", MFD_DEV_NAME, __func__, s2mu106->irq, s2mu106->irq_gpio);
	...
	for (i = 0; i < S2MU106_IRQ_NR; i++) {
		...
		irq_set_chip_and_handler(cur_irq, &s2mu106_irq_chip, handle_level_irq);
	...
	pr_info("%s: irq gpio pre-state(0x%02x)\n", __func__, gpio_get_value(s2mu106->irq_gpio));
...
EXPORT_SYMBOL_GPL(s2mu106_irq_init);
...
EXPORT_SYMBOL_GPL(s2mu106_irq_exit);
