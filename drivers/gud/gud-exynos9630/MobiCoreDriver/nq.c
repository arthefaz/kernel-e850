...
/* Limit the number of TEE workers; this is mostly for code consistency when */
/* building worker names and sysfs attributes. There is actually no other    */
/* technical reason to limit the number of TEE workers.                      */
/* BUILD_BUG_ON(NQ_TEE_WORKER_THREADS > 8); */
...
static u16 get_required_workers(void)
{
...
static int irq_bh_worker(void *arg)
{
	...
	while (1) {
		...
		/* FIXME SMP: this may be needed to properly handle FIQs */
		if (!get_workers())
...
static int tee_worker_wait(void)
{
	...
	ret = wait_event_interruptible(l_ctx.workers_wq,
...
static void tee_worker_count(int id, int resp)
{
	if (id >= NQ_TEE_WORKER_THREADS)
		mc_dev_err(-EINVAL, "worker id out of range (%d)!", id);

	/* FIXME: use atomic writes on counter values */

	switch (resp) {
	case MC_SMC_S_YIELD:
		l_ctx.tee_worker_counters[id].yield++;
		break;
	case MC_SMC_S_BUSY:
		l_ctx.tee_worker_counters[id].busy++;
		break;
	case MC_SMC_S_RESUME:
		l_ctx.tee_worker_counters[id].resume++;
		break;
	case MC_SMC_S_HALT:
		l_ctx.tee_worker_counters[id].halt++;
		break;
	default:
		l_ctx.tee_worker_counters[id].unknown++;
		break;
	}
}

static int tee_schedule(uintptr_t arg)
{
	...
	for (;;) {
		...
		/* FIXME SMP: possibly relax calls to logging because    */
		/* all workers may put pressure if they're all woken up. */
		logging_run();

		/* Count returned event for current worker */
		tee_worker_count(id, resp.resp);

		switch (resp.resp) {
		case MC_SMC_S_YIELD:
			/* FIXME SMP: resp.code returns -1 (infinite), -2,  */
			/* 0, or min. timeout value of internal time queue. */
				     ...
				     size_t buffer_len, loff_t *ppos)
{
	...
	/* FIXME: use atomic reads on counter values */
	...
	for (cnt = 0; cnt < NQ_TEE_WORKER_THREADS; cnt++) {
		ret = snprintf(worker_buffer,
			       ...
			       "unknown: %llu\n",
			       cnt,
			       l_ctx.tee_worker_counters[cnt].yield,
			       l_ctx.tee_worker_counters[cnt].busy,
			       l_ctx.tee_worker_counters[cnt].resume,
			       l_ctx.tee_worker_counters[cnt].halt,
			       l_ctx.tee_worker_counters[cnt].unknown);
		if (ret <= 0)
			break;
		strncat(l_ctx.struct_workers_counters_buf, worker_buffer,
			sizeof(l_ctx.struct_workers_counters_buf));
	}
...
static int tee_worker(void *arg)
{
	...
	for (;;) {
		...
		mc_dev_devel("[%ld] wake up run=%d", id, get_workers());
		ret = tee_schedule(id);
		if (ret)
			break;
	}
...
void nq_stop(void)
{
	...
	/* FIXME SMP: wait exit completion from all workers? */
					...
					size_t buffer_len, loff_t *x)
{
	...
	mc_dev_info("set tee_affinity 0x%lx", tee_affinity);
...
int nq_init(void)
{
	...
	/*FIXME: use default affinity from platform header if exists */
