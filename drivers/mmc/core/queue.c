					...
					struct mmc_queue *mq)
{
	...
	wait_event(ctx->wait, kthread_should_stop()
		|| (mmc_peek_request(mq) &&
		!(((req_op(mq->cmdq_req_peeked) == REQ_OP_FLUSH) || (req_op(mq->cmdq_req_peeked) == REQ_OP_DISCARD))
...
int mmc_cmdq_init(struct mmc_queue *mq, struct mmc_card *card)
{
	...
	for (i = 0; i < q_depth; i++) {
		mq->mqrq_cmdq[i].sg = mmc_alloc_sg(card->host->max_segs, GFP_KERNEL);
		if (!mq->mqrq_cmdq[i].sg) {
 ...
 */
void mmc_queue_suspend(struct mmc_queue *mq, int wait)
{
	struct request_queue *q = mq->queue;
	unsigned long flags;
	struct mmc_card *card = mq->card;
