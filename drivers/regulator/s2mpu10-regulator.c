...
static struct dentry *s2mpu10_root = NULL;
static struct dentry *s2mpu10_i2caddr = NULL;
static struct dentry *s2mpu10_i2cdata = NULL;
#endif
...
static int s2m_set_voltage_sel_regmap(struct regulator_dev *rdev, unsigned sel)
{
	...
	voltage = ((sel & rdev->desc->vsel_mask) * S2MPU10_LDO_STEP2) + S2MPU10_LDO_MIN1;
...
*/
	...
	_BUCK(_STEP1), _REG(_B7OUT2), _REG(_B7CTRL), _TIME(_BUCK7)),
	BUCK_DESC("BUCK8", _BUCK(8), &_buck_ops(), _BUCK(_MIN1),
	_BUCK(_STEP1), _REG(_B8OUT2), _REG(_B8CTRL), _TIME(_BUCK8)),
	BUCK_DESC("BUCK9", _BUCK(9), &_buck_ops(), _BUCK(_MIN2),
	_BUCK(_STEP2), _REG(_B9OUT2), _REG(_B9CTRL), _TIME(_BUCK9)),
};
#ifdef CONFIG_OF
...
#ifdef CONFIG_DEBUG_FS
static ssize_t s2mpu10_i2caddr_read(struct file *file, char __user *user_buf,
...
static ssize_t s2mpu10_i2caddr_write(struct file *file, const char __user *user_buf,
					size_t count, loff_t *ppos)
{
	char buf[10];
	ssize_t len;
...
static ssize_t s2mpu10_i2cdata_read(struct file *file, char __user *user_buf,
					size_t count, loff_t *ppos)
{
	char buf[10];
	ssize_t ret;

	ret = s2mpu10_read_reg(dbgi2c, i2caddr, &i2cdata);
	if (ret)
		return ret;
...
static ssize_t s2mpu10_i2cdata_write(struct file *file, const char __user *user_buf,
					size_t count, loff_t *ppos)
{
	char buf[10];
	ssize_t len, ret;
	...
	if (!kstrtou8(buf, 0, &val)) {
		ret = s2mpu10_write_reg(dbgi2c, i2caddr, val);
		if (ret < 0)
			return ret;
	}
...
static const struct file_operations s2mpu10_i2caddr_fops = {
	.open = simple_open,
	.read = s2mpu10_i2caddr_read,
	.write = s2mpu10_i2caddr_write,
	.llseek = default_llseek,
};
static const struct file_operations s2mpu10_i2cdata_fops = {
	.open = simple_open,
	.read = s2mpu10_i2cdata_read,
	.write = s2mpu10_i2cdata_write,
	.llseek = default_llseek,
};
#endif
...
#ifdef CONFIG_DEBUG_FS
	dbgi2c = s2mpu10->i2c;
	s2mpu10_root = debugfs_create_dir("s2mpu10-regs", NULL);
...
#ifdef CONFIG_DEBUG_FS
	debugfs_remove_recursive(s2mpu10_i2cdata);
	debugfs_remove_recursive(s2mpu10_i2caddr);
	debugfs_remove_recursive(s2mpu10_root);
#endif
