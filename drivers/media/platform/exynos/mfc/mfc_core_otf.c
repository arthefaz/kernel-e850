...
int mfc_core_otf_register_cb(struct mfc_ctx *ctx)
{
#if IS_ENABLED(CONFIG_VIDEO_EXYNOS_REPEATER)
	mfc_debug(2, "[OTF] ready for encoding\n");
		...
		struct repeater_encoding_param *param)
{
	...
	mfc_core_otf_ctx_ready_set_bit(core_ctx, &core->work_bits, true);
...
{
#if IS_ENABLED(CONFIG_VIDEO_EXYNOS_CAMERA_POSTPROCESS_GDC)
	...
	mfc_debug(2, "[vOTF] try MFC ready inode: %ld, gdc ready inode: %ld\n",
	...
	mfc_debug(2, "[vOTF] run with gdc ready inode: %ld\n", i_ino);
	ctx->gdc_ready_buf_ino = 0;
...
#if IS_ENABLED(CONFIG_VIDEO_EXYNOS_CAMERA_POSTPROCESS_GDC)
static int __mfc_core_votf_ready(unsigned long i_ino)
{
	struct mfc_dev *dev = g_mfc_dev;
	...
	if (src_mb) {
		if (!mfc_check_mb_flag(src_mb, MFC_FLAG_ENC_SRC_VOTF)) {
			mfc_ctx_err("[vOTF] gdc ready buffer(%ld) isn't vOTF\n", i_ino);
			return -EINVAL;
		}
	...
	if (ctx->gdc_ready_buf_ino) {
		mfc_ctx_err("[vOTF] privious buf(inode: %ld) that wasn't running will be ignored\n",
				ctx->gdc_ready_buf_ino);
...
int mfc_core_votf_init(struct mfc_ctx *ctx)
{
	int ret = -EFAULT;
...
#if IS_ENABLED(CONFIG_VIDEO_EXYNOS_CAMERA_POSTPROCESS_GDC)
	mfc_debug(2, "[vOTF] register GDC call back function\n");
	ret = gdc_register_ready_frame_cb(__mfc_core_votf_ready);
#endif
	if (ret)
		mfc_ctx_err("[VOTF] failed to register gdc encode call back function (ret: %d)\n",
				ret);
