	...
	for (i = 0; i < scenario_cnt; i++) {
		...
		if ((scenarios[i].check_func(device, NULL, position, resol, fps,
			...
			dbg_dvfs(1, "%s: pos(%d), res(%d), fps(%d), sc(%d), map(%d), dual(%d, %d, %d), scenario(%d)\n",
				...
				dual_info->max_fps_master,
				dual_info->max_fps_slave,
				dual_info->max_fps_slave2,
	...
	struct is_frame *frame)
{
	...
	case SENSOR_POSITION_REAR3:
		dual_info->max_fps_slave2 = frame->shot->ctl.aa.aeTargetFpsRange[1];
		break;
	default:
		return;
	}
	 ...
	 */
	if (dual_info->max_fps_master >= 24 &&
		dual_info->max_fps_slave == 0 &&
		dual_info->max_fps_slave2 == 0)
		dual_info->mode = IS_DUAL_MODE_BYPASS;
	else if (dual_info->max_fps_master >= 24 &&
		(dual_info->max_fps_slave >= 24 ||
		dual_info->max_fps_slave2 >= 24))
		dual_info->mode = IS_DUAL_MODE_SYNC;
	else if (dual_info->max_fps_master <= 5 &&
		(dual_info->max_fps_slave >= 24 ||
		dual_info->max_fps_slave2 >= 24))
