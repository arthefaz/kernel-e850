	...
	struct param_mcs_output *output, u32 output_id, u32 instance)
{
	...
	if (src_pos_x % MCSC_WIDTH_ALIGN) {
		mswarn_hw("[OUT:%d] hw_mcsc_poly_phase: src_pos_x should be multiple of %d(%d->%d)\n",
			...
			ALIGN_DOWN(src_pos_x, MCSC_WIDTH_ALIGN));
	...
	if (src_width % MCSC_WIDTH_ALIGN) {
		mswarn_hw("[OUT:%d] hw_mcsc_poly_phase: src_width should be multiple of %d(%d->%d)\n",
			...
			ALIGN_DOWN(src_width, MCSC_WIDTH_ALIGN));
	...
	if (poly_dst_width % MCSC_WIDTH_ALIGN) {
		mswarn_hw("[OUT:%d] hw_mcsc_poly_phase: poly_dst_width should be multiple of %d(%d->%d)\n",
			...
			ALIGN_DOWN(poly_dst_width, MCSC_WIDTH_ALIGN));
	...
	struct param_mcs_output *output, u32 output_id, u32 instance)
{
	...
	if (img_width % MCSC_WIDTH_ALIGN) {
		mswarn_hw("[OUT:%d] hw_mcsc_post_chain: img_width should be multiple of %d(%d->%d)\n",
			...
			ALIGN_DOWN(img_width, MCSC_WIDTH_ALIGN));
	...
	if (dst_width % MCSC_WIDTH_ALIGN) {
		mswarn_hw("[OUT:%d] hw_mcsc_post_chain: dst_width should be multiple of %d(%d->%d)\n",
			...
			ALIGN_DOWN(dst_width, MCSC_WIDTH_ALIGN));
	...
	u32 output_id, u32 instance)
{
	...
	if (output->hwfc)
		is_scaler_set_hwfc_config(hw_ip->regs[REG_SETA], output_id, format, plane,
			(output_id * HWFC_DMA_PLANE_OFFSET), width, height);
					...
					u32 *img_format, bool *conv420_flag)
{
	...
	switch (bitwidth) {
	/* bitwidth : 8 bit */
	case DMA_OUTPUT_BIT_WIDTH_8BIT:
		switch (format) {
		...
		default:
			...
			break;
	/* bitwidth : 10 bit */
	case DMA_OUTPUT_BIT_WIDTH_10BIT:
		switch (format) {
		case DMA_OUTPUT_FORMAT_YUV420:
			if (conv420_flag)
			*conv420_flag = true;
		...
		break;
	/* bitwidth : 16 bit */
	case DMA_OUTPUT_BIT_WIDTH_16BIT:
		switch (format) {
		case DMA_OUTPUT_FORMAT_YUV420:
			if (conv420_flag)
				*conv420_flag = true;
	...
	u32 width, u32 height)
{
	...
	case HW_MCSC_DMA_OUTPUT:
		...
		if (!(format == DMA_OUTPUT_FORMAT_YUV444
