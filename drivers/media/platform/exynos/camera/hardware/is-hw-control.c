...
u32 get_hw_id_from_group(u32 group_id)
{
	...
	default:
		hw_id = DEV_HW_END;
		err_hw("invalid group(%d)(%s)", group_id, group_id_name[group_id]);
		break;
	}
	...
	struct is_frame *frame)
{
	...
	memcpy(hw_frame->kvaddr_ext, frame->kvaddr_ext, sizeof(frame->kvaddr_ext));
	hw_frame->ext_planes = frame->ext_planes;
	...
	memcpy(hw_frame->ixtTargetAddress, frame->ixtTargetAddress, sizeof(frame->ixtTargetAddress));
	memcpy(hw_frame->ixgTargetAddress, frame->ixgTargetAddress, sizeof(frame->ixgTargetAddress));
	memcpy(hw_frame->ixvTargetAddress, frame->ixvTargetAddress, sizeof(frame->ixvTargetAddress));
	memcpy(hw_frame->ixwTargetAddress, frame->ixwTargetAddress, sizeof(frame->ixwTargetAddress));
	memcpy(hw_frame->mexcTargetAddress, frame->mexcTargetAddress, sizeof(frame->mexcTargetAddress));
	memcpy(hw_frame->orbxcTargetAddress, frame->orbxcTargetAddress, sizeof(frame->orbxcTargetAddress));
	...
	memcpy(hw_frame->clxsTargetAddress, frame->clxsTargetAddress, sizeof(frame->clxsTargetAddress));
	memcpy(hw_frame->clxcTargetAddress, frame->clxcTargetAddress, sizeof(frame->clxcTargetAddress));
	...
	struct is_group *group, struct is_frame *frame, ulong hw_map)
{
	...
	if ((hw_ip->internal_fcount[instance] >= frame->fcount || ret == INTERNAL_SHOT_EXIST)
		&& test_bit(IS_GROUP_OTF_INPUT, &head->state)) {
	...
	is_set_hw_count(hardware, head, instance, frame->fcount, frame->num_buffers, hw_map);
...
int make_internal_shot(struct is_hw_ip *hw_ip, u32 instance, u32 fcount,
	struct is_frame **in_frame, struct is_framemgr *framemgr)
{
	int ret = 0;
	int i = 0;
	struct is_frame *frame;
...
retry_get_frame:
	...
	if (!IS_ERR_OR_NULL(frame)) {
		if (unlikely(frame->fcount < fcount)) {
			...
			if (is_hardware_frame_ndone(hw_ip, frame, instance,
						IS_SHOT_UNPROCESSED)) {
	...
	ret = is_hardware_shot(hardware, instance, head,
			frame, framemgr, hardware->hw_map[instance], fcount);
	if (ret) {
...
void check_late_shot(struct is_hw_ip *hw_ip, u32 fcount)
{
	struct is_frame *frame;
	struct is_framemgr *framemgr = hw_ip->framemgr_late;
	...
	struct is_group *group, u32 instance, u32 next_id)
{
	...
	child = group->tail;
	while (child && (child->device_type == IS_DEVICE_ISCHAIN)) {
		curr_hw_id = get_hw_id_from_group(child->id);
		hw_slot = is_hw_slot_id(curr_hw_id);
		if (!valid_hw_slot_id(hw_slot)) {
			merr_hw("[ID:%d]invalid curr hw_slot_id [SLOT:%d]", instance,
				curr_hw_id, hw_slot);
			return -EINVAL;
		}
...
static int check_frame_end(struct is_hw_ip *hw_ip, u32 hw_fcount,
	struct is_frame **in_frame, struct is_framemgr *framemgr,
	u32 output_id, enum ShotErrorType done_type)
{
	int ret = 0;
	struct is_frame *frame = *in_frame;
	char *type = (output_id == IS_HW_CORE_END) ? "CORE" : "FRAM";
	ulong flags = 0;
	...
	if (hw_fcount > frame->fcount) {
		...
		ret = is_hardware_frame_ndone(hw_ip, frame,
...
struct is_hw_ip *is_get_hw_ip(u32 id, struct is_hardware *hardware)
{
	struct is_hw_ip *hw_ip = NULL;
	enum is_hardware_id hw_id = DEV_HW_END;
	...
	hw_maxnum = is_get_hw_list(id, hw_list);
	hw_id = hw_list[0];
	hw_slot = is_hw_slot_id(hw_id);
	if (!valid_hw_slot_id(hw_slot)) {
		err_hw("invalid slot (%d,%d)", hw_id, hw_slot);
	...
	int wq_id, u32 output_id, enum ShotErrorType done_type, bool get_meta)
{
	...
	hw_fe_cnt = atomic_read(&hw_ip->count.fe);
	...
	if (output_id == IS_HW_CORE_END) {
		...
		case IS_SHOT_SUCCESS:
			if (!test_bit_variables(hw_ip->id, &frame->core_flag)) {
				ret = check_frame_end(hw_ip, hw_fe_cnt, &frame,
	...
	enum ShotErrorType done_type)
{
	...
	while (head) {
		hw_maxnum = is_get_hw_list(head->id, hw_list);
		for (hw_index = 0; hw_index < hw_maxnum; hw_index++) {
			hw_id = hw_list[hw_index];
			hw_slot = is_hw_slot_id(hw_id);
			if (!valid_hw_slot_id(hw_slot)) {
