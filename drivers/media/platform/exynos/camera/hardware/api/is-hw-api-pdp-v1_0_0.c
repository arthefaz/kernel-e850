...
static void _pdp_hw_s_lic_ch0(void __iomem *base, u32 curr_ch, u32 curr_path, struct pdp_lic_lut *lut)
{
	...
	if (lut) {
		...
		if (mode == PDP_LIC_MODE_DYNAMIC) {
			...
			val = PDP_SET_V(val, PDP_F_LIC_WEIGHT_CONTEXT_0, 1);
			val = PDP_SET_V(val, PDP_F_LIC_WEIGHT_CONTEXT_1, 1);
			val = PDP_SET_V(val, PDP_F_LIC_WEIGHT_CONTEXT_2, 1);
...
void pdp_hw_s_one_shot_enable(void __iomem *base)
{
	...
	do {
		...
		dbg_hw(2, "[PDP] %s (total:%d, curr:%d, idle:%d)\n",
			__func__, total_line, curr_line, idle);
	} while (!idle);
