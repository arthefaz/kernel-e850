...
#include "is-dt.h"
#include "is-cis-5e9.h"
#include "is-cis-5e9-setA.h"
#include "is-cis-5e9-setB.h"
...
int sensor_5e9_cis_init(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_write8(client, 0xA00, 0x04);
	if (unlikely(ret)) {
		err("failed to is_i2c_write (%d)\n", ret);
		ret = -EINVAL;
		goto p_err;
	}
	ret = is_sensor_write8(client, 0xA02, 0x02);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	ret = is_sensor_write8(client, 0xA00, 0x01);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);

	ret = is_sensor_read8(client, 0xA12, &selected_page);
	if (unlikely(ret)) {
		err("failed to is_i2c_read (%d)\n", ret);
		ret = -EINVAL;
		goto p_err;
	...
	if (selected_page == 0x3) {
		...
		if (unlikely(ret))
			err("failed to is_i2c_read (%d)\n", ret);
		ret = is_sensor_write8(client, 0xA00, 0x00);
		if (unlikely(ret))
			err("failed to is_i2c_read (%d)\n", ret);
		...
		if (unlikely(ret))
			err("failed to is_i2c_read (%d)\n", ret);
		ret = is_sensor_write8(client, 0xA02, 0x03);
		if (unlikely(ret))
			err("failed to is_i2c_read (%d)\n", ret);
		ret = is_sensor_write8(client, 0xA00, 0x01);
		if (unlikely(ret))
	...
	ret = is_sensor_read16(client, 0x0A04, &data16[0]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	ret = is_sensor_read16(client, 0x0A06, &data16[1]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	ret = is_sensor_read16(client, 0x0A08, &data16[2]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	ret = is_sensor_read16(client, 0x0A0A, &data16[3]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);

	ret = is_sensor_read8(client, 0xA22, &cal_map_ver[0]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	ret = is_sensor_read8(client, 0xA23, &cal_map_ver[1]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	ret = is_sensor_read8(client, 0xA24, &cal_map_ver[2]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	ret = is_sensor_read8(client, 0xA25, &cal_map_ver[3]);
	if (unlikely(ret))
		err("failed to is_i2c_read (%d)\n", ret);
	...
	if (skip_cal_write == false) {
		ret = is_sensor_write16_array(client, 0x020E, data16, 4);
		if (ret < 0) {
			printk(KERN_INFO "is_sensor_write16_array fail\n");
			ret = -EINVAL;
			goto p_err;
		}
	}
...
int sensor_5e9_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (is_vender_wdr_mode_on(cis_data))
		ret = is_sensor_write8(client, 0x0216, 0x01);
	else
		ret = is_sensor_write8(client, 0x0216, 0x00);
...
int sensor_5e9_cis_set_digital_gain(struct v4l2_subdev *subdev, struct ae_param *dgain)
{
	...
	/* Long digital gain */
	if (is_vender_wdr_mode_on(cis_data)) {
		ret = is_sensor_write16(client, 0x305C, long_gain);
		if (ret < 0)
			goto p_err;
	}
...
int sensor_5e9_cis_get_min_digital_gain(struct v4l2_subdev *subdev, u32 *min_dgain)
{
	...
	ret = is_sensor_read16(client, 0x1084, &read_value);
	if (unlikely(ret))
...
int sensor_5e9_cis_get_max_digital_gain(struct v4l2_subdev *subdev, u32 *max_dgain)
{
	...
	ret = is_sensor_read16(client, 0x1086, &read_value);
	if (unlikely(ret))
