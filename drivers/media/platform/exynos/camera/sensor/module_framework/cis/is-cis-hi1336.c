...
int sensor_hi1336_cis_check_rev(struct is_cis *cis)
{
	int ret    = 0;
	u16 data16 = 0;
	...
	ret = is_sensor_read16(client, SENSOR_HI1336_MODEL_ID_ADDR, &data16);
	if (ret < 0) {
...
int sensor_hi1336_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_read16(client, SENSOR_HI1336_STREAM_ONOFF_ADDR, &data16);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
...
static int sensor_hi1336_cis_group_param_hold_func(struct v4l2_subdev *subdev, bool hold)
{
	...
	ret = is_sensor_read16(client, SENSOR_HI1336_GROUP_PARAM_HOLD_ADDR, &hold_set);
	if (ret < 0) {
	...
	ret = is_sensor_write16(client, SENSOR_HI1336_GROUP_PARAM_HOLD_ADDR, hold_set);
	if (ret < 0) {
...
int sensor_hi1336_cis_enable_frame_cnt(struct i2c_client *client, bool enable)
{
	...
	ret = is_sensor_read8(client, SENSOR_HI1336_MIPI_TX_OP_MODE_ADDR,
			&frame_cnt_ctrl);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
	...
	ret = is_sensor_write8(client, SENSOR_HI1336_MIPI_TX_OP_MODE_ADDR,
			frame_cnt_ctrl);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
				SENSOR_HI1336_MIPI_TX_OP_MODE_ADDR, ret);
...
int sensor_hi1336_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	ret = is_sensor_write16(client, SENSOR_HI1336_STREAM_ONOFF_ADDR,
			0x0001);
	I2C_MUTEX_UNLOCK(cis->i2c_lock);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
...
int sensor_hi1336_cis_wait_streamoff(struct v4l2_subdev *subdev)
{
	...
	ret = is_sensor_read16(client, SENSOR_HI1336_ISP_FRAME_CNT_ADDR,
			&cur_frame_value);
	if (ret < 0) {
		err("i2c transfer fail addr(%x) ret = %d\n",
