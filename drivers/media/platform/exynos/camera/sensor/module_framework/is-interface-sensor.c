...
#include "pafstat/is-hw-pafstat.h"
#endif
...
/* helper functions */
static struct is_module_enum *get_subdev_module_enum(struct is_sensor_interface *itf)
{
...
static struct is_device_sensor *get_device_sensor(struct is_sensor_interface *itf)
{
...
static struct is_device_csi *get_subdev_csi(struct is_sensor_interface *itf)
{
		...
		camera2_udm_t *udm)
{
	...
	if (sensor_peri->cis.cis_data->cur_lownoise_mode == IS_CIS_LN4)
		dm->sensor.rollingShutterSkew = sensor_peri->cis.cis_data->rolling_shutter_skew * 4;
	else if (sensor_peri->cis.cis_data->cur_lownoise_mode == IS_CIS_LN2)
