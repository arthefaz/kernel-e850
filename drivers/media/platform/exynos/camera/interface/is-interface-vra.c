...
static void is_lib_vra_callback_frw_abort(void)
{
	dbg_lib(3, "vra_callback_frw_abort\n");
}

static void is_lib_vra_callback_frw_hw_err(u32 ch_index, u32 err_mask)
{
	err_lib("callback_frw_hw_err: ch_index(%#x), err_mask(%#x)",
		ch_index, err_mask);
}

static void is_lib_vra_callback_final_output_ready(u32 instance,
	...
	const struct api_vra_out_list_info *out_list_info_ptr)
{
	...
	if (IS_ENABLED(ENABLE_REPROCESSING_FD)) {
		...
		if (test_bit(instance, &lib_vra->done_vra_hw_intr)
			...
			is_hardware_frame_done(lib_vra->hw_ip, NULL, -1,
...
static void is_lib_vra_callback_crnt_fr_output_ready(u32 instance,
...
static void is_lib_vra_callback_post_detect_ready(u32 instance,
...
int __nocfi is_lib_vra_set_post_detect_output(struct is_lib_vra *lib_vra,
...
static void is_lib_vra_callback_end_input(u32 instance,
		u32 frame_index, unsigned char *base_address)
{
	dbg_lib(3, "lib_vra_callback_end_input: base_addr(%p)\n", base_address);
}

static void is_lib_vra_callback_frame_error(u32 instance,
...
int is_lib_vra_invoke_contol_event(struct is_lib_vra *lib_vra)
{
	enum api_vra_type status = VRA_NO_ERROR;
...
void is_lib_vra_set_event_fw_algs(u32 event_type)
{
	struct is_lib_vra *lib_vra = g_lib_vra;

	switch (event_type) {
	case FWALGS_TASK_SET_ABORT:
		/* This event is processed in is_lib_vra_fwalgs_stop */
...
int __nocfi is_lib_vra_alloc_memory(struct is_lib_vra *lib_vra, ulong dma_addr)
{
	...
	for (index = 0; index < VRA_TOTAL_SENSORS; index++) {
		...
		lib_vra->frame_desc_heap[index] = is_alloc_vra(size);
...
int is_lib_vra_free_memory(struct is_lib_vra *lib_vra)
{
	...
	for (index = 0; index < VRA_TOTAL_SENSORS; index++)
		is_free_vra(lib_vra->frame_desc_heap[index]);
	...
	void __iomem *base_addr, enum is_lib_vra_input_type input_type)
{
	...
	callbacks->frw_abort_func_ptr     = is_lib_vra_callback_frw_abort;
	callbacks->frw_hw_err_func_ptr    = is_lib_vra_callback_frw_hw_err;
	callbacks->sen_final_output_ready_ptr = is_lib_vra_callback_final_output_ready;
	callbacks->sen_crnt_fr_output_ready_ptr = is_lib_vra_callback_crnt_fr_output_ready;
	callbacks->sen_end_input_proc_ptr = is_lib_vra_callback_end_input;
	callbacks->sen_error_ptr          = is_lib_vra_callback_frame_error;
	callbacks->sen_stat_collected_ptr = NULL;
#if defined(ENABLE_HYBRID_FD)
	callbacks->frw_invoke_hybrid_pr_ptr	= NULL;
	callbacks->frw_abort_hybrid_pr_ptr	= NULL;
	callbacks->sen_post_detect_ready_ptr	= is_lib_vra_callback_post_detect_ready;
#else
	callbacks->frw_invoke_hybrid_pr_ptr	= NULL;
	callbacks->frw_abort_hybrid_pr_ptr	= NULL;
...
int __nocfi is_lib_vra_init_frame_desc(struct is_lib_vra *lib_vra, u32 instance)
{
	...
	void __iomem *base_addr, enum is_lib_vra_input_type input_type)
{
	...
	if (!test_bit(VRA_LIB_FRAME_WORK_INIT, &lib_vra->state)) {
		ret = is_lib_vra_init_frame_work(lib_vra, base_addr, input_type);
		if (ret) {
	...
	u32 instance)
{
	...
	if (!test_bit(VRA_INST_FRAME_DESC_INIT, &lib_vra->inst_state[instance])) {
		ret = is_lib_vra_init_frame_desc(lib_vra, instance);
		if (ret) {
...
int __nocfi is_lib_vra_set_orientation(struct is_lib_vra *lib_vra,
...
static int is_lib_vra_fwalgs_stop(struct is_lib_vra *lib_vra)
{
	...
	ret = is_lib_vra_invoke_fwalgs_event(lib_vra);
	if (ret) {
	...
	struct camera2_stats_dm *dm)
{
	...
	if (IS_ENABLED(ENABLE_HYBRID_FD))
		dbg_lib(3, "lib_vra:%s:num(%d)\n", __func__, lib_vra->pdt_all_face_num[instance]);
...
#ifdef ENABLE_HYBRID_FD
static int is_lib_vra_update_hfd_dm(struct is_lib_vra *lib_vra, u32 instance,
	struct hfd_meta *hfd)
{
	int face_num;
	struct api_vra_face_base_str *base;
...
#ifdef ENABLE_HYBRID_FD
	ret = is_lib_vra_update_hfd_dm(lib_vra, frame->instance,
		&frame->shot_ext->hfd);
	if (ret) {
...
void __nocfi is_lib_vra_os_funcs(void)
{
	...
	funcs.control_task_set_event = is_lib_vra_set_event_control;
	funcs.fw_algs_task_set_event = is_lib_vra_set_event_fw_algs;
...
void is_lib_vra_check_size(struct api_vra_input_desc *frame_desc, struct vra_param *param, u32 fcount)
{
...
int __nocfi is_lib_vra_otf_input(struct is_lib_vra *lib_vra,
...
int __nocfi is_lib_vra_apply_tune(struct is_lib_vra *lib_vra,
	struct is_lib_vra_tune_data *vra_tune, u32 instance)
{
	...
	struct is_lib_vra_frame_lock *info_frame;
	...
	if (!vra_tune || dma_test) {
		...
		memcpy(&tune, vra_tune, sizeof(struct is_lib_vra_tune_data));
...
#ifdef ENABLE_VRA_CHANGE_SETFILE_PARSING
int __nocfi is_lib_vra_copy_tune_set(struct is_lib_vra *lib_vra,
...
int __nocfi is_lib_vra_apply_tune_set(struct is_lib_vra *lib_vra,
...
int __nocfi is_lib_vra_reset_recovery(struct is_lib_vra *lib_vra,
		u32 instance_id)
{
	int ret = 0;
