...
struct s2mf301_muic_data *g_muic_data;
    ...
    t_path_data path_data)
{
	...
	muic_data->path_status = path_data;
...
static int s2mf301_if_get_path_status(void *mdata)
{
	struct s2mf301_muic_data *muic_data = (struct s2mf301_muic_data *)mdata;
	u8 reg_val;
	...
	reg_val &= S2MF301_MANUAL_SW_CTRL_SWITCH_MASK;

	if (reg_val & (1 << 0))
		return MUIC_PATH_USB_AP;
	else if (reg_val & (1 << 1))
		return MUIC_PATH_UART_AP;
	else if (reg_val & (1 << 2))
		return MUIC_PATH_UART_CP;
	else
		return MUIC_PATH_OPEN;
}
