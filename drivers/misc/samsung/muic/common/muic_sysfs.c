...
/* CAUTION : Do not be declared as external s2m_class  */
static struct class *s2m_class;
static atomic_t s2m_dev;
...
struct device *s2m_device_create(void *drvdata, const char *fmt)
{
	struct device *dev;

	if (IS_ERR(s2m_class)) {
		pr_err("Failed to create class(s2m) %ld\n", PTR_ERR(s2m_class));
		BUG();
	}

	if (!s2m_class) {
		pr_err("Not yet created class(s2m)!\n");
		BUG();
	}

	dev = device_create(s2m_class, NULL, atomic_inc_return(&s2m_dev),
			drvdata, fmt);
	if (IS_ERR(dev))
...
EXPORT_SYMBOL_GPL(s2m_device_create);
...
int muic_sysfs_init(struct muic_platform_data *muic_pdata)
{
	...
	s2m_class = class_create(THIS_MODULE, "s2m");
	if (IS_ERR(s2m_class)) {
		pr_err("Failed to create class(s2m) %ld\n", PTR_ERR(s2m_class));
		return PTR_ERR(s2m_class);
	}

	mutex_init(&muic_pdata->sysfs_mutex);

	if (muic_pdata->switch_device == NULL)
		muic_pdata->switch_device = s2m_device_create(NULL, "switch");
