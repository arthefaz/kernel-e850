...
#endif
struct ifconn_manager_data *ifconn_manage;

int ifconn_manager_get_muic_cable_type(void)
{
	return ifconn_manage->muic_cable_type;
}

int ifconn_manager_get_pd_con_state(void)
{
	return ifconn_manage->pd_con_state;
}

static struct ifconn_notifier_ops manager_ops = {
	.get_muic_cable_type = ifconn_manager_get_muic_cable_type,
	.get_pd_con_state = ifconn_manager_get_pd_con_state,
};
				     ...
				     struct ifconn_notifier_template *template)
{
	...
	case IFCONN_NOTIFY_MUIC:
		...
		template->src = IFCONN_NOTIFY_MANAGER_MUIC;
	...
	case IFCONN_NOTIFY_CCIC:
		...
		//template->src = IFCONN_NOTIFY_MANAGER;
		template->src = IFCONN_NOTIFY_MANAGER_PDIC;
		template->data = NULL;
				      ...
				      struct ifconn_notifier_template *template)
{
	...
	case IFCONN_NOTIFY_ID_POWER_STATUS:
		if (template->attach || template->event == IFCONN_NOTIFY_EVENT_ATTACH) {
...
{
	struct ifconn_manager_data *ifconn_manager = ifconn_manage;
	struct ifconn_notifier_template *template =
...
#endif
	ifconn_manage = ifconn_manager;
	ifconn_notifier_set_ops(&manager_ops);

	for (i = IFCONN_NOTIFY_USB; i <= IFCONN_NOTIFY_USB_DP; i++) {
		ret = ifconn_notifier_register(&ifconn_manager->nb[i],
					       ifconn_manager_isr,
