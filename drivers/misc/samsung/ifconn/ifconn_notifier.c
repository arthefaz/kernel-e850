			     ...
			     ifconn_notifier_t listener, ifconn_notifier_t src)
{
	...
	if (pnoti[src].sent[listener] || pnoti[src].sent[IFCONN_NOTIFY_ALL]) {
		pr_info("%s: src : %d, listner : %d, id : %d, sent : %d, sent all : %d\n", __func__,
			src, listener, pnoti[src].ifconn_template[listener].id, pnoti[src].sent[listener],
			pnoti[src].sent[IFCONN_NOTIFY_ALL]);
		...
		if (listener == IFCONN_NOTIFY_BATTERY) {
			if (notifier_ops.get_pd_con_state()) {
				...
				if (template->id == IFCONN_NOTIFY_ID_DETACH) {
