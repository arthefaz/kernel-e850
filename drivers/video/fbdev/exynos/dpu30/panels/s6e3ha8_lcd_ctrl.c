...
void lcd_init(int id, struct decon_lcd *lcd)
{
	...
	if (dsim_drvdata[0]->board_info == 1) {
		msleep(5);
...
{
	if (dsim_drvdata[0]->board_info == 1) {
		dsim_info("board info(%d) : s6e3ha9\n", dsim_drvdata[0]->board_info);
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)KEY1_ENABLE_HA9,
					ARRAY_SIZE(KEY1_ENABLE_HA9)) < 0) {
			dsim_err("failed to write KEY1_ENABLE\n");
			return;
		}
		if (dsc_en) {
			if (dsim_wr_data(id, MIPI_DSI_DSC_PRA,
						(unsigned long)DSC_EN_HA9[1][0], 0) < 0) {
				dsim_err("failed to write DSC_EN\n");
				return;
			}
			if (dsim_wr_data(id, MIPI_DSI_DSC_PPS,
						(unsigned long)PPS_TABLE_HA9[mres_idx],
						ARRAY_SIZE(PPS_TABLE_HA9[mres_idx])) < 0) {
				dsim_err("failed to write PPS_TABLE[%d]\n", mres_idx);
				return;
			}
		} else {
			if (dsim_wr_data(id, MIPI_DSI_DSC_PRA,
						(unsigned long)DSC_EN_HA9[0][0], 0) < 0) {
				dsim_err("failed to write DSC_EN\n");
				return;
			}
		}

		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)KEY1_DISABLE_HA9,
					ARRAY_SIZE(KEY1_DISABLE_HA9)) < 0) {
			dsim_err("failed to write KEY1_DISABLE\n");
			return;
		}
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE,
					(unsigned long)CASET_TABLE_HA9[mres_idx],
					ARRAY_SIZE(CASET_TABLE_HA9[mres_idx])) < 0) {
			dsim_err("failed to write CASET_TABLE[%d]\n", mres_idx);
			return;
		}
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE,
					(unsigned long)PASET_TABLE_HA9[mres_idx],
					ARRAY_SIZE(PASET_TABLE_HA9[mres_idx])) < 0) {
			dsim_err("failed to write PASET_TABLE[%d]\n", mres_idx);
			return;
		}
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)KEY2_ENABLE_HA9,
					ARRAY_SIZE(KEY2_ENABLE_HA9)) < 0) {
			dsim_err("failed to write KEY2_ENABLE\n");
			return;
		}
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE,
					(unsigned long)SCALER_TABLE_HA9[mres_idx],
					ARRAY_SIZE(SCALER_TABLE_HA9[mres_idx])) < 0) {
			dsim_err("failed to write SCALER_TABLE[%d]\n", mres_idx);
			return;
		}
		if (dsim_wr_data(id, MIPI_DSI_DCS_LONG_WRITE, (unsigned long)KEY2_DISABLE_HA9,
					ARRAY_SIZE(KEY2_DISABLE_HA9)) < 0) {
			dsim_err("failed to write KEY2_DISABLE\n");
			return;
		}
	} else {
		dsim_info("board info(%d) : s6e3ha8\n", dsim_drvdata[0]->board_info);
