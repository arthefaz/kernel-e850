...
int set_otg_notify(struct otg_notify *n)
{
	...
	u_notify->notifier_wq
		= alloc_ordered_workqueue("usb_notify", WQ_FREEZABLE | WQ_MEM_RECLAIM);
...
int otg_wake_lock_ctrl(int onoff)
{
	struct otg_notify *o_notify;
	struct usb_notify *u_notify;
	...
	o_notify = get_otg_notify();
	if (o_notify == NULL) {
		pr_info("%s : Can't get otg_notify!\n", __func__);
		return -1;
	}
	...
	if (onoff) {
		if (o_notify->is_wakelock) {
			pr_info("Lock otg wakeup lock\n");
			__pm_stay_awake(u_notify->ws);
		}
	} else {
		if (o_notify->is_wakelock) {
			pr_info("Unlock otg wakeup lock\n");
			__pm_relax(u_notify->ws);
		}
	}
...
EXPORT_SYMBOL(otg_wake_lock_ctrl);
