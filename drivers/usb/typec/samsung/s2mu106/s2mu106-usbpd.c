...
static void s2mu106_usbpd_set_rp_scr_sel(struct s2mu106_usbpd_data *pdic_data, PDIC_RP_SCR_SEL scr_sel);
...
void s2mu106_usbpd_authentic(void *_data)
{
    ...
    s2mu106_usbpd_read_reg(i2c, S2MU106_REG_PLUG_CTRL_VBUS_MUX, &reg);
    reg &= ~(S2MU106_REG_RD_OR_VBUS_MUX_SEL);
    s2mu106_usbpd_write_reg(i2c, S2MU106_REG_PLUG_CTRL_VBUS_MUX, reg);
}
...
void s2mu106_rprd_mode_change(struct s2mu106_usbpd_data *usbpd_data, u8 mode)
{
	...
	case TYPE_C_ATTACH_DFP: /* SRC */
		...
		usbpd_data->status_reg |= 1LL << PLUG_ATTACH;
																...
																int mode)
{
	...
	if (psy_pm) {
		...
		ret = psy_pm->desc->set_property(psy_pm,
							(enum power_supply_property)POWER_SUPPLY_S2M_PROP_CO_ENABLE, &val);
	} else {
...
static int s2mu106_get_side_check(void *_data)
{
	...
	cc1_val = (val & S2MU106_REG_CTRL_MON_CC1_MASK) >> S2MU106_REG_CTRL_MON_CC1_SHIFT;
	cc2_val = (val & S2MU106_REG_CTRL_MON_CC2_MASK) >> S2MU106_REG_CTRL_MON_CC2_SHIFT;
...
int s2mu106_get_plug_monitor(struct s2mu106_usbpd_data *pdic_data, u8 *data)
{
	...
	data[0] = (reg_val & S2MU106_REG_CTRL_MON_CC1_MASK) >> S2MU106_REG_CTRL_MON_CC1_SHIFT;
		...
		unsigned long action, void *data)
{
	...
	struct s2mu106_usbpd_data *pdic_data =
		container_of(nb, struct s2mu106_usbpd_data, type3_nb);
	...
	struct s2mu106_usbpd_data *pdic_data)
{
	...
	mutex_lock(&pdic_data->lpm_mutex);
	usbpd_wake_lock(pd_data->usbpd_ws);
	if (!pdic_data->lpm_mode) {
	...
	usbpd_wake_unlock(pd_data->usbpd_ws);
	mutex_unlock(&pdic_data->lpm_mutex);
}
...
{
	struct s2mu106_usbpd_data *pdic_data = i2c_get_clientdata(i2c);
...
#if IS_ENABLED(CONFIG_DUAL_ROLE_USB_INTF)
		devm_dual_role_instance_unregister(pdic_data->dev,
						pdic_data->dual_role);
		devm_kfree(pdic_data->dev, pdic_data->desc);
#elif IS_ENABLED(CONFIG_TYPEC)
		typec_unregister_port(pdic_data->port);
#endif
		disable_irq_wake(pdic_data->i2c->irq);
		free_irq(pdic_data->i2c->irq, pdic_data);
		i2c_set_clientdata(pdic_data->i2c, NULL);
		kfree(pdic_data);
	}
...
static usbpd_phy_ops_type s2mu106_ops = {
    ...
    .usbpd_authentic    = s2mu106_usbpd_authentic,
...
{
	int ret = 0;
	ret = i2c_add_driver(&s2mu106_usbpd_driver);
	pr_info("%s, (%d)\n", __func__);
	return ret;
}
...
MODULE_LICENSE("GPL");
MODULE_SOFTDEP("pre: i2c-exynos5 s2mu106_charger s2m_muic_module");
