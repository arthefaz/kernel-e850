...
#define CHECK_MSG(pd, msg, ret) do {\
	if (PDIC_OPS_PARAM_FUNC(get_status, pd, msg))\
		return ret;\
	} while (0);
...
const char *vdm_command_type_to_str[] = {
	"REQ",
	"ACK",
	"NAK",
	"BUSY",
...
policy_state usbpd_policy_src_send_capabilities(struct policy_data *policy)
{
	...
	**********************************************/
	...
	while (1) {
		...
		ms = usbpd_check_time1(pd_data);
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_REQUEST)) {
		...
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_GET_SNK_CAP)) {
			dev_info(pd_data->dev, "%s got MSG_GET_SNK_CAP.\n", __func__);
			ret = PE_SRC_Send_Soft_Reset;
			break;
		}

		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_ERROR)) {
			ret = PE_SRC_Discovery;
			break;
		}

		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_PING)) {
			ret = PE_SRC_Send_Soft_Reset;
			break;
		}
		...
		if (ms >= tSenderResponse) {
			if (received_goodcrc) {
				dev_info(pd_data->dev, "%s tSenderResponse Timer over -> go PE_SRC_Hard_Reset.\n", __func__);
				CHECK_MSG(pd_data, MSG_GET_SNK_CAP, PE_SRC_Send_Soft_Reset);
...
policy_state usbpd_policy_src_negotiate_capability(struct policy_data *policy)
{
	...
	**********************************************/
	...
	if ((usbpd_manager_match_request(pd_data) == 0) || policy->send_reject == 1) {
...
policy_state usbpd_policy_src_ready(struct policy_data *policy)
{
	...
	**********************************************/
	...
	if (data_role == USBPD_DFP && pd_data->dp_enabled)
	...
	**********************************************/
	...
	while (1) {
		if (policy->plug_valid == 0)
			break;
		ms = usbpd_check_time1(pd_data);
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_SNK_CAP)) {
...
policy_state usbpd_policy_src_send_soft_reset(struct policy_data *policy)
{
	...
	**********************************************/
	...
	while (1) {
		...
		ms = usbpd_check_time1(pd_data);
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_ACCEPT)) {
			ret = PE_SRC_Send_Capabilities;
			break;
		}
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_ERROR)) {
...
policy_state usbpd_policy_src_soft_reset(struct policy_data *policy)
{
	...
	**********************************************/
	...
	while (1) {
		...
		ms = usbpd_check_time1(pd_data);
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_GOODCRC)) {
			dev_info(pd_data->dev, "Rx MSG_GOODCRC\n");
			ret = PE_SRC_Send_Capabilities;
			break;
		}

		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_ERROR)) {
			dev_info(pd_data->dev, "Occur MSG_ERROR\n");
			ret = PE_SRC_Hard_Reset;
			break;
		}
		if (ms >= 10) {
...
policy_state usbpd_policy_snk_select_capability(struct policy_data *policy)
{
	...
	**********************************************/
	...
	while (1) {
		...
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_REJECT)) {
		...
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_WAIT)) {
			/* 1st Power Negotiation Check */
			if (pd_noti.sink_status.selected_pdo_num == 0)
				ret = PE_SNK_Wait_for_Capabilities;
			else
...
policy_state usbpd_policy_snk_transition_sink(struct policy_data *policy)
{
	...
	**********************************************/
	...
	while (1) {
		...
		if (PDIC_OPS_PARAM_FUNC(get_status, pd_data, MSG_PSRDY)) {
			/* Device Information */
...
policy_state usbpd_policy_snk_hard_reset(struct policy_data *policy)
{
	...
	**********************************************/
	...
	pr_info("%s, time out sec = %ld, usec = %ld\n",
		__func__, pd_data->check_time.tv_sec, pd_data->check_time.tv_nsec * 1000);
	PDIC_OPS_FUNC(hard_reset, pd_data);
	PDIC_OPS_PARAM_FUNC(set_pd_control, pd_data, USBPD_CC_OFF);
...
static policy_state usbpd_policy_dfp_vdm_evaluate(struct policy_data *policy)
{
	...
	pr_info("%s, vdm [0x%x], vdm(0x%x), type(0x%x:%s)\n", __func__,
			pd_data->protocol_rx.data_obj[0].object, vdm_command, vdm_type,
			vdm_command_type_to_str[vdm_type]);

	switch (vdm_command) {
	case Discover_Identity:
		if (vdm_type == Responder_ACK) {
			if (is_timeout)
	...
	case Attention:
		return PE_DFP_VDM_Attention_Request;
	}
