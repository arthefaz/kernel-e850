/*
 * sound/soc/codec/aud3004x-5pin.c
 *
...
#define AUD3004X_ADC_THD_FAKE_JACK			321
#define AUD3004X_ADC_THD_AUXCABLE			551
#define AUD3004X_ADC_THD_WATER_IN			671
#define AUD3004X_ADC_THD_WATER_OUT			3101

/* Default: 1.57V */
#define AUD3004X_LDET_VTH_DEFAULT			LDET_VTH_1_57V
/* Default: 2000K */
#define AUD3004X_LDET_POP_DEFAULT			LDET_POP_2000K
 ...
 * @adc_mode: If it checks LDET adc, adc_mode is 0. If it checks MDET adc,
 * adc_mode is 1.
 *
 * Desc: When something is inserted at ear-jack house, the codec checks
 * LDET adc data for separate the water or the ear-jack at first.
...
static void aud3004x_jack_det_work(struct work_struct *work)
{
	...
	if (jackstate->cur_jack_state & (JACK_POLE_DEC | JACK_AUX | JACK_SURGE_OUT)) {
		...
		if (!fake_jack_check(jackdet)) {
			queue_delayed_work(jackdet->ldet_adc_wq, &jackdet->ldet_adc_work,
					msecs_to_jiffies(100));
...
static int return_ldet_adc_thd(struct aud3004x_jack *jackdet)
{
	struct earjack_state *jackstate = &jackdet->jack_state;

	/* Detect the type of inserted jack using ldet adc */
	if (jackstate->ldet_adc < jackdet->adc_thd_fake_jack)
		return JACK_RET_JACK;
	else if (jackstate->ldet_adc >= jackdet->adc_thd_fake_jack &&
			jackstate->ldet_adc < jackdet->adc_thd_auxcable)
		return JACK_RET_FAKE;
	else if (jackstate->ldet_adc >= jackdet->adc_thd_auxcable &&
	...
	else if (jackstate->ldet_adc >= jackdet->adc_thd_water_out)
		return JACK_RET_OUT;
	else
		return JACK_RET_ERR;
}
...
/*
 * read_ldet_adc() - Get ldet adc data
 *
 * @jackdet: jackdet information struct
 *
 * Desc: Read ldet adc value.
 */
static void read_ldet_adc(struct aud3004x_jack *jackdet)
{
	struct aud3004x_priv *aud3004x = jackdet->p_aud3004x;
	struct earjack_state *jackstate = &jackdet->jack_state;

	if (jackdet->ldet_pop == LDET_POP_2000K) {
		aud3004x_regcache_switch(aud3004x, false);

		/* LDET Pull-up registance 500k */
		aud3004x_write(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_D1_DCTR_TEST1, 0x10);
		aud3004x_update_bits(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_C1_ACTR_JD2,
				CTRV_LDET_POP_MASK, LDET_POP_500K);

		jackstate->ldet_adc = adc_get_value(jackdet, 0);

		/* LDET Pull-up registance 2M */
		aud3004x_update_bits(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_C1_ACTR_JD2,
				CTRV_LDET_POP_MASK, LDET_POP_2000K);
		aud3004x_write(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_D1_DCTR_TEST1, 0x00);

		aud3004x_regcache_switch(aud3004x, true);
	} else {
		/*
		 * If tuning the LDET Pull-up registance,
		...
		aud3004x_write(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_D1_DCTR_TEST1, 0x10);
		aud3004x_update_bits(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_C1_ACTR_JD2,
				CTRV_LDET_POP_MASK, LDET_POP_500K << CTRV_LDET_POP_SHIFT);
		jackstate->ldet_adc = adc_get_value(jackdet, 0);
		aud3004x_update_bits(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_C1_ACTR_JD2,
				CTRV_LDET_POP_MASK, jackdet->ldet_pop << CTRV_LDET_POP_SHIFT);
...
/*
 * aud3004x_ldet_adc_work() - Process AP check interrupt
 *
 * @work: delayed workqueue
 *
 * Desc: When AP check interrupt is occured, need to check ldet adc value for
 * determine if it is jack or water. If it is jack, call jack_det_work process.
 * If it is water, set water polling configuration.
 */
static void aud3004x_ldet_adc_work(struct work_struct *work)
{
	struct aud3004x_jack *jackdet =
		container_of(work, struct aud3004x_jack, ldet_adc_work.work);
	struct earjack_state *jackstate = &jackdet->jack_state;
	struct device *dev = jackdet->p_aud3004x->dev;

	mutex_lock(&jackdet->ldet_lock);

	/* read ldet adc value */
	read_ldet_adc(jackdet);

	dev_dbg(dev, "%s called, ldet adc: %d\n", __func__, jackstate->ldet_adc);

	switch (return_ldet_adc_thd(jackdet)) {
	case JACK_RET_JACK:
	...
	mutex_unlock(&jackdet->ldet_lock);
}
...
static void aud3004x_jack_parse_dt(struct aud3004x_priv *aud3004x)
{
	...
	int bias_v_conf, mic_range, mic_open_value, btn_rel_val, delay, ant_ldet_delay;
	int jackin_dbnc, jackout_dbnc, ant_ldetin_dbnc, ant_ldetout_dbnc;
	int thd_adc, ldet_value;
	int i, ret;
	...
	/* LDET adc check delay time */
	ret = of_property_read_u32(dev->of_node, "ldet-delay", &delay);
	if (!ret)
		jackdet->ldet_delay = delay;
	else
		jackdet->ldet_delay = AUD3004X_LDET_DELAY;
	...
	dev_dbg(dev, "LDET delay: %d, MDET delay: %d, ANT LDET delay: %d, Mic adc range: %d,"
			" Mic open value: %d\n", jackdet->ldet_delay, jackdet->mdet_delay,
			jackdet->ant_ldet_delay, jackdet->mic_adc_range, jackdet->mic_open_value);
	...
	dev_dbg(dev, "LDET adc threshold value: %d %d %d %d\n",
			jackdet->adc_thd_fake_jack,
			jackdet->adc_thd_auxcable,
			jackdet->adc_thd_water_in,
			jackdet->adc_thd_water_out);

	/* Set LDET VTH and Pull-up */
	ret = of_property_read_u32(dev->of_node, "ldet-vth", &ldet_value);
	if (!ret)
		jackdet->ldet_vth = ldet_value;
	else
		jackdet->ldet_vth = AUD3004X_LDET_VTH_DEFAULT;

	ret = of_property_read_u32(dev->of_node, "ldet-pop", &ldet_value);
	if (!ret)
		jackdet->ldet_pop = ldet_value;
	else
		jackdet->ldet_pop = AUD3004X_LDET_POP_DEFAULT;

	dev_dbg(dev, "LDET VTH: %d, LDET POP: %d\n",
			jackdet->ldet_vth, jackdet->ldet_pop);
...
static void aud3004x_jack_register_initialize(struct snd_soc_component *codec)
{
	...
	/* LDET VTH Setting */
	aud3004x_update_bits(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_C1_ACTR_JD2,
			CTRV_LDET_VTH_MASK, jackdet->ldet_vth << CTRV_LDET_VTH_SHIFT);
	...
	aud3004x_update_bits(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_D0_DCTR_CM,
			EN_FIVE_MASK, EN_FIVE_MASK);
#if IS_ENABLED(CONFIG_SND_SOC_AUD3004X_EXT_ANT)
	/* Antenna */
	aud3004x_update_bits(aud3004x, AUD3004X_DIGITAL_ADDR, AUD3004X_D0_DCTR_CM,
			EN_ANT_MASK, EN_ANT_MASK);
#endif
...
//	struct snd_soc_component *codec = jackdet->codec;
	...
	case IRQ_ST_APCHECK:
		...
		queue_delayed_work(jackdet->ldet_adc_wq, &jackdet->ldet_adc_work,
				msecs_to_jiffies(jackdet->ldet_delay));
		break;
	case IRQ_ST_JACKDET:
...
EXPORT_SYMBOL_GPL(aud3004x_call_notifier);
...
int aud3004x_jack_probe(struct snd_soc_component *codec)
{
	...
	INIT_DELAYED_WORK(&jackdet->ldet_adc_work, aud3004x_ldet_adc_work);
	jackdet->ldet_adc_wq = create_singlethread_workqueue("ldet_adc_wq");
	if (jackdet->ldet_adc_wq == NULL) {
		dev_err(codec->dev, "Failed to create ldet_adc_wq\n");
		return -ENOMEM;
	}
...
int aud3004x_jack_remove(struct snd_soc_component *codec)
{
	...
	destroy_workqueue(jackdet->ldet_adc_wq);
