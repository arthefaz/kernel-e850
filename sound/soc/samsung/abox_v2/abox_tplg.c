/* sound/soc/samsung/abox_v2/abox_tplg.c
 *
...
static int abox_tplg_widget_load(struct snd_soc_component *cmpnt,
		int index,
		struct snd_soc_dapm_widget *w,
		struct snd_soc_tplg_dapm_widget *tplg_w)
{
...
static int abox_tplg_widget_ready(struct snd_soc_component *cmpnt,
		int index,
		struct snd_soc_dapm_widget *w,
		struct snd_soc_tplg_dapm_widget *tplg_w)
{
...
static int abox_tplg_link_load(struct snd_soc_component *cmpnt,
		int index,
...
static int abox_tplg_manifest(struct snd_soc_component *cmpnt,
		int index,
...
#else
	ret = request_firmware(&abox_tplg_fw, fw_name, dev);
	for (i = retry; ret && i; i--) {
		msleep(1000);
		ret = request_firmware(&abox_tplg_fw, fw_name, dev);
	}
